...
class Produto(models.Model):
    MARCAS_DISPONIVEIS_CHOICES = (
        ("oboticario", "OBoticario"),
        ("natura", "Natura"),
        ("eudora", "Eudora"),
        ("blossom", "Blossom"),
    )
    ...
    categoria = models.CharField(max_length=20, choices=CATEGORIA_CHOICES, default='perfume')  # novo campo
    ...
    def save(self, *args, **kwargs):
        try:
            this = Produto.objects.get(id=self.id)
            if this.foto != self.foto and this.foto:
                if os.path.isfile(this.foto.path):
